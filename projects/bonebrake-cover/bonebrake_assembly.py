...
# carried by hand into Fusion program names / posted gcode (e.g. face-revA.nc).
...
sheet, and the gcode posted from Fusion (name programs `…-{REV_TAG}.nc`). Before cutting,
...
Post programs named to match the rev — `{BASENAME}-face-{REV_TAG}.nc`, `{BASENAME}-middle-{REV_TAG}.nc` — and attach to the PPBO Job, removing superseded revs.
