...
OUT_STEP = OUT_DIR / f"{BASENAME}-assembly.step"
...
def print_cut_list() -> None:
    tap = TAP_DRILL_DIA
    relief = RELIEF_DIA
    print("\nFRAME CUT LIST — 0.25 x", BAR, "flat bar, 45-deg miters both ends")
...
def write_bom(out_path: Path) -> Path:
    ...
    lines = [
        f"# Bonebrake Cover — BOM  ({W:.2f} x {H:.2f})",
        "",
...
def write_buildsheet(out_path: Path) -> Path:
    ...
    text = f"""# Bonebrake Cover — Build Sheet  ({W:.2f} x {H:.2f})

Generated by bonebrake_assembly.py — regenerate, don't hand-edit.
Companion docs: bonebrake-drillmap.svg (hole letters referenced below), bonebrake-bom.md.
...
- Drill AFTER welding, per drill map: 4x C ({frac(TAP_DRILL_DIA)}") at corners -> tap {BOLT};
  6x D ({frac(RELIEF_DIA)}") rivet reliefs. Positions on the drill map / drill schedule.
...
def main() -> None:
    ...
    for panel_name, sk in (
        ...
        dxf_path = OUT_STEP.parent / f"{BASENAME}-{panel_name}.dxf"
    ...
    map_path = drill_map(
        ...
        out_path=OUT_STEP.parent / f"{BASENAME}-drillmap.svg",
        title=f"Bonebrake Cover — Drill Map  ({W:.2f} x {H:.2f} test coupon)",
    ...
    bom_path = write_bom(OUT_STEP.parent / f"{BASENAME}-bom.md")
    print(f"wrote {bom_path.name}")
    sheet_path = write_buildsheet(OUT_STEP.parent / f"{BASENAME}-buildsheet.md")
    print(f"wrote {sheet_path.name}")

    asm = Compound(children=[frame, middle, face, *fasteners])
    asm.label = "bonebrake-cover"
