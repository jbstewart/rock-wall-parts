# Bonebrake Cover — Build Sheet  (Rev B, 9.00 x 41.25)

Generated by bonebrake_assembly.py at Rev B — regenerate, don't hand-edit.
Companion docs: bonebrake-drillmap-revB.pdf (hole letters referenced below), bonebrake-bom-revB.md.

**Revision rule:** every file for this job carries `revB` in its filename — DXFs, this
sheet, and the gcode posted from Fusion (name programs `…-revB.gcode`). Before cutting,
check the filename on the ArcDroid screen ends in `revB`; if it doesn't, or two revs of
...
Material: 1/4 x 1.5" flat bar, mild steel. Cut per cut list — 45-deg miters both ends:
- 2 pcs long-point 41.250" (sides)
- 2 pcs long-point 9.000" (top/bottom)

Requirements (the what — technique is yours):
- Finished frame 9.000 x 41.250 outside, square within 1/32 across diagonals, FLAT
...
- Drill AFTER welding, per drill map: 4x C (17/64") at corners -> tap 5/16-18;
  10x D (11/32") rivet reliefs. Positions on the drill map / drill schedule.
...
|---|---|---|---|---|
| bonebrake-face-revB.gcode | 11ga mild steel | **1** | 9" | 41.5" |
| bonebrake-middle-revB.gcode | 11ga mild steel | **1** | 9" | 41.5" |
...
- Shop fit check: set riveted unit on frame — shop heads drop into the D reliefs,
  unit sits FLAT on the frame. Run all 4 bolts (5/16-18 x 1/2 flanged button, 91255A578)
